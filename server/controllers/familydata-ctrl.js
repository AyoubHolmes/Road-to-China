...
updateFamilyData = async (req, res) => {
    ...
    FamilyData.findOne({ _id: req.params.id }, (err, familydata) => {
        if (err)
...
deleteFamilyData = async (req, res) => {
    await FamilyData.findOneAndDelete({ _id: req.params.id }, (err, familyData) => {
        if(err)
...
getFamilyDataById = async (req, res) => {
    await FamilyData.findOne({ _id: req.params.id }, (err, familyData) => {
        if(err)
            return res.status(400).json({err});
        if(!familyData)
            return res.status(404).json({success: false, error: 'familyData not found'});
